...
- **Commit messages must never include a `Co-Authored-By:` trailer.** Not for Claude Opus, not for Claude Sonnet, not for any model. The commit message is exactly what the user approved, nothing more. This overrides the default Claude Code behavior of appending a Co-Authored-By trailer to git commits.
