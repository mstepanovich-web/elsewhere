...
| 019_trivia_premium_usage.sql | ❓ Verify pending | — | — | Trivia Phase 2 (Commit A) rate-limit table. Required by the `generate-trivia` Edge Function for the per-user per-UTC-day 20-generations limit. Apply via Supabase SQL Editor before deploying the Edge Function — function will return 500 "rate-limit lookup failed" if the table doesn't exist. Verify post-application via `SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'trivia_premium_usage');` returning `t`, then flip this row to ✅ in a follow-up doc-only commit. |
