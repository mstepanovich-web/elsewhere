...
| 021_session_set_admission_mode.sql | ⏳ Drafted, not yet applied | — | — | Implements W2 from `docs/ADMISSION-MODEL-V2.md` § 10. New manager-only SECURITY DEFINER RPC `rpc_session_set_admission_mode(p_session_id, p_admission_mode, p_capacity)` for stamping admission_mode + capacity onto the sessions row at game-start time (and clearing on Switch Game). Validates `p_admission_mode` against the same set as `sessions_admission_mode_check` (NULL/'open'/'gated'); errors 22023 on invalid value, 42501 on non-manager caller, 02000 on missing/ended session. Bumps `last_activity_at` on success. Called from `games/player.html`'s `managerStartTrivia` / `managerStartLastCard` / `managerStartEuchre` / `managerSwitchGame` (W2 client-side wiring lands in the same commit as this migration). Apply after review; verification queries in the migration file footer. |
