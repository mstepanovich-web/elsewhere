...
| 020_admission_model_v2.sql | ⏳ Drafted, not yet applied | — | — | Implements W1 from `docs/ADMISSION-MODEL-V2.md` § 10. Additive schema changes only: adds `session_participants.wanting_since` (nullable timestamptz), relaxes `sessions.admission_mode` NOT NULL + CHECK constraint to allow NULL plus the new values `'open'`/`'gated'` only, transitions existing rows with old admission_mode values to NULL, backfills `wanting_since = joined_at` for pre-existing queued rows. Does NOT drop `queue_position` or its index (karaoke depends on them per § 2.7); does NOT rewrite any RPCs (W2 scope). Apply after review; verification queries in the migration file footer. |
