...
| `rpc_session_end` | db/009 | **Participant sweep must STOP.** Currently sets `left_at = now()` on all active participants. Per the room/session split, ending a session no longer ends membership in the room. New body: set `ended_at` + `last_activity_at` only; DO NOT touch session_participants. Room ending DOES sweep participants — but it lands inside `rpc_session_leave`'s no-eligible-successor branch (db/028, see the `rpc_session_leave` row below), not here. An explicit user-initiated "close room" affordance is a separate Phase-5 concern; the empty-room branch in `rpc_session_leave` is the only room-ending code path in Phase-1. |
| `rpc_session_leave` | db/010 | **Three changes.** (1) Re-point all session_id filters → room_id; signature changes from `(p_session_id uuid)` to `(p_room_id uuid, p_successor_user_id uuid DEFAULT NULL)`. (2) Add the new optional `p_successor_user_id` parameter to support the named-successor tier. (3) Rewrite successor selection per ROOM-AUTHORITY-MODEL.md's four-tier hierarchy (HOST-FIRST — corrected by the same commit that lands db/028): **(Tier 1)** if any active host (`control_role = 'host'`, `left_at IS NULL`, not the leaver) is present, promote the longest continuously-present host (by `joined_at asc LIMIT 1`) — applies to BOTH explicit and implicit departure, a host always wins; **(Tier 2)** else if `p_successor_user_id` is non-null AND resolves to an eligible active room participant (`left_at IS NULL`, `participation_role IN ('active', 'queued')`, not the leaver), promote them — silent fall-through to tier 3 if ineligible; **(Tier 3)** else promote the longest continuously-present non-audience participant (`participation_role IN ('active', 'queued')`, `left_at IS NULL`, not the leaver, by `joined_at asc LIMIT 1`); **(Tier 4)** if no tier matched, end the ROOM. The `joined_at asc` ordering is correct for "longest continuously-present" because per the existing row-creation contract (`db/008:114-116`), a rejoin gets a new row with a new `joined_at` — the column captures the participant's current unbroken stint. **The tier-4 room-end is the FIRST and ONLY writer of `rooms.ended_at` in the entire system** — the structural counterpart to `rpc_room_create`'s sole-writer guarantee on `rooms.owner_user_id` (db/027). Write order is FIXED: (1) end the active session if any (`sessions.ended_at = now()`), (2) sweep participants (`session_participants.left_at = now()`), (3) LAST, set `rooms.controller_user_id = NULL` and `rooms.ended_at = now()`. Contents are cleaned up before the room is stamped ended, so no reader ever sees an ended room with live contents. **Critical: this RPC transfers room CONTROL only — it must NOT touch `rooms.owner_user_id`.** Even on room-end, ownership is preserved — the convener retains their saved-room template binding (ROOM-AUTHORITY-MODEL.md § "Room ownership"). |
