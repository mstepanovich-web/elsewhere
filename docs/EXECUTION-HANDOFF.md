...
**Last updated:** 2026-05-28 (Phase-3 / Plan B Stage A4a ship + closeout. Stage A4a — 2D-canvas spotlight anchor renderer + 3-kind authoring panel + db/037 3-anchor seed — shipped 2026-05-27 (implementation `f167ec6`, mid-verification fixes `0b2dec0` + `2056a72`, closeout this commit). A4a is the 2D-canvas-only sub-stage of A4 per planning chat 2026-05-27; A4b ships 3D spotlight builders + absorbed 3D particle paths + new Three.js admin preview surface as a separate propose-pause cycle. All six §8 verification checks (Checks 19–24) PASS; A4a verification log at `docs/SESSION-LOGS/2026-05-27-A4a-verification-result.md`. Three bugs caught + fixed mid-verification, all attributable to §9 step 4 proposal drift from A3 patterns. **Stage A4b** (3D spotlight + 3D particle extension + Three.js admin preview surface) is next per `docs/VENUE-ADMIN-UI-DIRECTION.md` §7. §2 enumerates the workstream's current status; §4 names the next deliverable.)
...
- **Stage A3 (particle renderer + kind-discriminated authoring panel + db/036 4-anchor seed + karaoke registration tag)**: implementation `e9c52e9` (2026-05-27); mid-verification kind-gate fix `27610e4`; closeout `01fc791`; verification log `docs/SESSION-LOGS/VENUE-ADMIN-UI-A3-VERIFICATION-LOG.md`. db/036 applied to prod 2026-05-26 (all 3 §2.3 verification queries Q1–Q3 PASS first run). All six §8 verification checks (Checks 13–18) PASS. One bug caught + fixed mid-verification (preview-ended label was arming on point-cloud — eternal kind — wrongly; gated on kind in `27610e4`). One schema-state divergence (Check 15 round-trip left stadium with panel-generated UUID id; restored to seed id `anc_par_stadium` via one-row SQL UPDATE before closeout). A3 is 2D-canvas only per spec §0.2 re-staging — 3D particle paths (stadium 2000 Three.js Points + speakeasy 60 sphere meshes) defer to Stage A4b. Six DEFERRED entries filed in the closeout.
- **Stage A4a (2D-canvas spotlight renderer + kind-discriminated authoring panel + db/037 3-anchor seed + karaoke registration tag)**: implementation `f167ec6` (2026-05-27); mid-verification fixes `0b2dec0` (admin-venues.html script tag — §9 step 4 coverage gap) + `2056a72` (RPC signature + status element class — §9 step 4 proposal drift from A3 patterns); closeout this commit; verification log `docs/SESSION-LOGS/2026-05-27-A4a-verification-result.md`. db/037 applied to prod 2026-05-27 (all 3 §2.3 verification queries Q1–Q3 PASS first run; Q3 split into 4 sub-queries due to Supabase SQL Editor's `limit 100` auto-append breaking the CASE-expression form — verification-mechanics artifact, not payload). All six §8 verification checks (Checks 19–24) PASS. Three bugs caught + fixed mid-verification, all attributable to §9 step 4 proposal drift from A3 patterns. One schema-state divergence (Check 21 round-trip left stadium with panel-generated UUID id; restored to seed id `anc_spot_stadium` via one-row SQL UPDATE before closeout). A4a is the 2D-canvas-only sub-stage of A4 per planning chat 2026-05-27 — 3D spotlight builders (stadium 4 cone meshes + speakeasy 40 candle Points), absorbed 3D particle paths (stadium 2000 phone-lights + speakeasy 60 sphere-mesh smoke), and new Three.js admin preview surface all defer to Stage A4b. Eight DEFERRED entries filed in the closeout (enumerated in §4).

Stages A4b–A8 (3D spotlight + 3D particle extension + Three.js admin preview surface; overlay renderer for disco floor-flash + festival strobe + honkytonk neon-tint at Stage A4.5; remaining anchor types at Stage 5; AMBIENT_PROFILES retirement at Stage A7; per-app override editor at Stage 6; costume editor at Stage A8) are queued per Direction §7 and not yet started. Block B (the karaoke reader-path rewire that retires `karaoke/stage.html`'s inline venue code) is folded into Stage A7.
...
**Phase 3 / Plan B Stages A1, A2, A3, A4a are all shipped and verified.** db/034 + db/035 + db/036 + db/037 + `admin-venues.html` (with audio + particle + spotlight panels) + `shell/venue-renderers/audio.js` + `shell/venue-renderers/particle.js` + `shell/venue-renderers/spotlight.js` + the three-line `karaoke/stage.html` registration block are live in prod. The admin UI is the working write path for `venue_defaults` edits, audio anchor authoring (19 venues), particle anchor authoring (4 venues), and spotlight anchor authoring (3 venues — stadium / festival / speakeasy 2D effects). All four stages verified against prod with all bugs caught (A3 Check 15; A4a Checks 19 + 21) fixed mid-verification before closeout. See verification logs at `docs/SESSION-LOGS/VENUE-ADMIN-UI-A1-STAGE-1-VERIFICATION-LOG.md` (`f610039`), `VENUE-ADMIN-UI-A1-STAGE-2-VERIFICATION-LOG.md` (`a1a02e3`), `VENUE-ADMIN-UI-A3-VERIFICATION-LOG.md` (`01fc791`), and `2026-05-27-A4a-verification-result.md` (this commit) for per-check evidence.

**The immediate next step is Stage A4b per `docs/VENUE-ADMIN-UI-DIRECTION.md` §7's "Plan B hybrid sequencing".** Stage A4b is the **3D spotlight + 3D particle extension + new Three.js admin preview surface** sub-stage of A4 (sub-staged into A4a + A4b in planning chat 2026-05-27 because 3D preview is genuinely new architectural territory — `admin-venues.html` has zero Three.js code today). A4b ships: (a) `swept-beam-3d` kind for stadium 4 cone meshes (`buildStadiumEffects3D`); (b) `point-light` kind for speakeasy 40 candle Points (`buildSpeakeasyEffects3D`); (c) the absorbed 3D particle paths deferred from A3 §0.2 (stadium 2000 `THREE.Points` phone-lights + speakeasy 60 sphere-mesh smoke clouds — extends `particle.js` with 3D-context handling, or introduces sibling kinds `point-cloud-3d` / `volumetric-3d`); (d) the new Three.js admin preview surface (currently zero in `admin-venues.html` — needs scene + camera + renderer + RAF loop scaffolding). The pattern set by A2/A3/A4a (renderer impl in `shell/venue-renderers/<type>.js`, registered via `window.elsewhere.anchorRegistry.registerAnchorRenderer`, with a matching authoring panel + per-type seed in a new `db/0NN_*.sql` migration) carries forward; A4b extends `spotlight.js` + `particle.js` rather than introducing a new renderer module. Stage A4b ships as its own propose-pause cycle.

**Numbering note for the next implementer.** Direction §7 calls spotlight **Stage 4** (now sub-staged into 4a + 4b). The A3 spec refers to spotlight as "Stage A5" in ~6-8 cross-references across §§0.2, 6, 7 — this is a numbering drift relative to Direction §7's authoritative staging. The A3 spec's "A5 (spotlight + 3D builders)" should be read as "A4" (and specifically "A4b" for the 3D builder portion). Captured as a DEFERRED entry for spec correction on next touch of that file.
...
- **Venue modulator system** — A3 records modulator bindings in payloads (name + target) but drives them with built-in preview oscillators in particle.js; real registry-resolved drivers deferred. Cross-cutting (drives particles AND spotlights); A4a deliberately ships modulator-free per D5, so the A7 driver implementation continues to carry only A3's bindings + the synthesized A4b 3D phone-lights binding (see A4a entry (a) below).
- **Particle panel validates JSON well-formedness only, not §1 schema shape** — well-formed-but-wrong-shape payloads pass the panel and reach the renderer, which degrades gracefully. Acceptable for admin-only tool; revisit if surface widens. (Spotlight panel inherits same posture per A4a.)
...
- **A3 build spec internal references to spotlight as "Stage A5" should be "Stage A4"** — documentation drift surfaced during the A3 closeout's "what's next" determination; mechanical find-and-replace pass, fold into next touch of that file. (Now also "Stage A4b" for the 3D portion per the A4 sub-staging.)

**Eight DEFERRED.md entries filed in the A4a closeout (this commit):**

- **(a) A4b 3D phone-lights modulator synthesis decision** — A4b binds `crowd_brightness` on the new stadium 3D phone-lights anchor for parity with A3's 2D phone-lights anchor; source uses frame-counter sin, so the binding is synthesized. A7's driver-registry inventory must include this synthesized name.
- **(b) PERMIT multi-anchor structural UI evolution** — when A4b ships speakeasy's 2nd spotlight anchor (`point-light` candles alongside `light-shaft`), the panel needs an anchor-list pattern with per-anchor editor (current "one kind at a time" form swap doesn't work for multi-kind-in-venue). Bundle with A4b panel restructuring.
- **(c) GSAP-equivalent motion accuracy verification gap** — renderer ease functions reproduce GSAP semantics within numerical precision; A7's read-path switch is the first prod exercise. Side-by-side verification deferred to A7 cycle.
- **(d) §9 step 4 proposal-vs-A3-implementation drift cluster (3 fixes)** — three bugs caught mid-verification (script tag missing at admin-venues.html; RPC called with 11-arg shape vs 3-arg p_partial jsonb; status element class invented as `.anchor-row-status` doesn't match A3's `.anchor-status`). All fixed at `0b2dec0` + `2056a72`. The PROCESS gap (proposal authoring should verify call shapes + DOM selectors against A3 source, not just English description) is the deferred item.
- **(e) Kind-switch UX divergence** — A4a panel re-renders on kind change (discards other-kind edits); A3 uses hide/show via `[data-kind-section]` (preserves edits). Functional but suboptimal UX. Bundle with A4b panel restructuring.
- **(f) Spotlight + particle panel button styling + Stop-all placement** — low-contrast button styling inconsistent with theme; "Stop all previews" at panel top vs. per-row previews at row bottom creates scroll-distance UX issue. UI polish.
- **(g) Missing per-row Stop button on preview** — both spotlight + particle panels currently provide only a global "Stop all" affordance. Per-row Stop would be cleaner. UI polish.
- **(h) A4a mid-implementation findings (resolved during implementation)** — three findings corrected inline during `f167ec6`: `state.spotlightAnchorDirty` Map→Set; pending-id pattern aligned to A3's `anc_<uuid>`; `state.spotlightAnchors` Map cache added beyond original proposal. No outstanding work; entry exists for inventory completeness.

**Still-active downstream items — tracked in `docs/DEFERRED.md`, NOT blocking Stage A4b:**
...
**Push + prod state.** All Phase-2 + Phase-3-Plan-B-A1+A2+A3+A4a commits pushed; `origin/main` at the A4a closeout commit (this commit). db/034 + db/035 + db/036 + db/037 applied to prod.
