...
- **Authoritative A1–A8 staging:** `docs/VENUE-ADMIN-UI-DIRECTION.md` §7 ("Plan B hybrid sequencing"). The A1 build spec at `docs/VENUE-ADMIN-UI-A1-BUILD-SPEC.md` (committed `cf2585c`, revised `8187c5d` + `647c31b`) holds the Stage A1 + Stage A2 fold-in detail. Stage A3 has its own build spec at `docs/VENUE-ADMIN-UI-A3-BUILD-SPEC.md` (committed `bc8c448`, §1 amendment `716746b`). PHASE-2-BUILD-SPEC.md §7 (the original admin-UI scope decision) remains authoritative for higher-level direction.
- **Sequencing — locked by Plan B:** admin UI skeleton first, then per-type vertical slices that build renderer impl + UI panel + translate the venues that use that type. Audio first (19 venues), particle next (4 venues — A3 foundation pass corrected the spec's "~6" estimate to 4 active in the 26-venue inventory; honkytonk's particles were deleted; ghost venues are out of scope), spotlight after (3 venues + the 2 Three.js 3D builders). `AMBIENT_PROFILES` + `addVenueEffects3D` retire from `karaoke/stage.html` once every procedural venue has a data-driven equivalent that renders visually identically. Block B follows — the karaoke reader-path rewire that switches `karaoke/stage.html` onto the shell renderer + registry, completing the Phase-3 karaoke half. Detail in Direction §7.
- **Status:** **In progress** (A1 + A2 + A3 + A4a shipped; A4b next).
    - **Stage A1** (admin UI skeleton + `venue_defaults` editor + db/034 `rpc_venue_default_update`) shipped 2026-05-26 — commits `9cf4b70` + path fix `606674f`; verification log `f610039`.
    - **Stage A2** (audio renderer impl `shell/venue-renderers/audio.js` + audio anchor authoring panel + 19-venue audio seed + db/035 `rpc_venue_anchor_upsert` / `rpc_venue_anchor_delete`) shipped 2026-05-26 — commit `9d58a8d`; verification log + A1 row-number correction `a1a02e3`.
    - **Stage A3** (2D-canvas particle renderer `shell/venue-renderers/particle.js` + kind-discriminated authoring panel + 4-venue particle seed in `db/036_particle_anchor_seed.sql` + karaoke registration tag) shipped 2026-05-27 — implementation `e9c52e9`; mid-verification kind-gate fix `27610e4`; closeout `01fc791`. All six §8 verification checks PASS. Verification log at `docs/SESSION-LOGS/VENUE-ADMIN-UI-A3-VERIFICATION-LOG.md`. A3 is 2D-canvas only per spec §0.2 — 3D particle paths (stadium 2000 Three.js Points + speakeasy 60 sphere meshes) defer to Stage A4b paired with the 3D builder work. Six DEFERRED entries filed in the closeout.
    - **Stage A4a** (2D-canvas spotlight renderer `shell/venue-renderers/spotlight.js` + kind-discriminated authoring panel + 3-venue spotlight seed in `db/037_spotlight_anchor_seed.sql` + karaoke + admin-venues.html registration tags) shipped 2026-05-27 — implementation `f167ec6`; mid-verification fixes `0b2dec0` (admin-venues.html script tag — §9 step 4 coverage gap) + `2056a72` (RPC signature + status element class — §9 step 4 proposal drift from A3 patterns); closeout this commit. All six §8 verification checks PASS. Verification log at `docs/SESSION-LOGS/2026-05-27-A4a-verification-result.md`. A4a is the 2D-canvas-only sub-stage of A4 per planning chat 2026-05-27 — A4b ships 3D spotlight builders + absorbed 3D particle paths + new Three.js admin preview surface. Eight DEFERRED entries filed in the closeout.
    - **db/034 + db/035 + db/036** applied to prod 2026-05-26; **db/037** applied to prod 2026-05-27.
    - **Stage A4b** (3D spotlight builders for stadium 4 cone meshes + speakeasy 40 candle Points; absorbed 3D particle paths for stadium 2000 phone-lights + speakeasy 60 sphere-mesh smoke; new Three.js admin preview surface in admin-venues.html) is the next deliverable per Direction §7.
- **Estimated:** TBD per stage. Each A-stage is its own propose-pause cycle.
- **Scope clarification:** the admin UI is the authoring/preview tool that mitigates the venue-translation risk per Plan B. The karaoke reader-path rewire (Block B) — the actual switch of `karaoke/stage.html` from inline venue code onto `shell/venue-registry`'s registered renderers — is folded into Stage A7. Games venue integration remains Phase 4.
- **Trigger for next stage:** continue per Plan-B sequencing. Stage A4b brief drafted at next planning-chat session.
- **References:** `docs/VENUE-ADMIN-UI-DIRECTION.md` (Plan B direction + authoritative §7 staging); `docs/VENUE-ADMIN-UI-A1-BUILD-SPEC.md` (Stage A1 + Stage A2 spec); `docs/VENUE-ADMIN-UI-A3-BUILD-SPEC.md` (Stage A3 spec); `docs/VENUE-ADMIN-UI-A4A-BUILD-SPEC.md` (Stage A4a spec); `docs/PHASE-2-BUILD-SPEC.md` §3-§7 (the data model + original admin UI scope); `db/032_venue_abstraction_schema.sql` (the tables); `db/034_venue_default_update_rpc.sql` + `db/035_audio_anchor_rpcs_and_seed.sql` + `db/036_particle_anchor_seed.sql` + `db/037_spotlight_anchor_seed.sql` (the per-stage migrations shipped to date); `shell/venue-settings.js` (the resolver the UI reads + writes through); `shell/venue-registry.js` (the registry the renderer impls are registered against); `shell/venue-renderers/audio.js` (Stage A2's renderer impl) + `shell/venue-renderers/particle.js` (Stage A3's renderer impl) + `shell/venue-renderers/spotlight.js` (Stage A4a's renderer impl — the pattern Stage A4b extends); per-stage verification logs in `docs/SESSION-LOGS/`.
