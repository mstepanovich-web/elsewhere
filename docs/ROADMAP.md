...
- **Status:** **In progress** (A1 + A2 + A3 shipped; A4 next).
    ...
    - **Stage A3** (2D-canvas particle renderer `shell/venue-renderers/particle.js` + kind-discriminated authoring panel + 4-venue particle seed in `db/036_particle_anchor_seed.sql` + karaoke registration tag) shipped 2026-05-27 — implementation `e9c52e9`; mid-verification kind-gate fix `27610e4`; closeout `01fc791`. All six §8 verification checks PASS. Verification log at `docs/SESSION-LOGS/VENUE-ADMIN-UI-A3-VERIFICATION-LOG.md`. A3 is 2D-canvas only per spec §0.2 — 3D particle paths (stadium 2000 Three.js Points + speakeasy 60 sphere meshes) defer to Stage A4 paired with the 3D builder work. Six DEFERRED entries filed in the closeout.
    - **db/034 + db/035 + db/036** all applied to prod 2026-05-26.
    - **Stage A4** (spotlight renderer + spotlight authoring panel + stadium/disco/speakeasy spotlight translations + the 2 Three.js 3D builders for stadium and speakeasy) is the next deliverable per Direction §7.
...
- **Trigger for next stage:** continue per Plan-B sequencing. Stage A4 brief drafted at next planning-chat session.
- **References:** `docs/VENUE-ADMIN-UI-DIRECTION.md` (Plan B direction + authoritative §7 staging); `docs/VENUE-ADMIN-UI-A1-BUILD-SPEC.md` (Stage A1 + Stage A2 spec); `docs/VENUE-ADMIN-UI-A3-BUILD-SPEC.md` (Stage A3 spec); `docs/PHASE-2-BUILD-SPEC.md` §3-§7 (the data model + original admin UI scope); `db/032_venue_abstraction_schema.sql` (the tables); `db/034_venue_default_update_rpc.sql` + `db/035_audio_anchor_rpcs_and_seed.sql` + `db/036_particle_anchor_seed.sql` (the per-stage migrations shipped to date); `shell/venue-settings.js` (the resolver the UI reads + writes through); `shell/venue-registry.js` (the registry the renderer impls are registered against); `shell/venue-renderers/audio.js` (Stage A2's renderer impl) + `shell/venue-renderers/particle.js` (Stage A3's renderer impl — the pattern Stage A4 mirrors); `docs/SESSION-LOGS/VENUE-ADMIN-UI-A1-STAGE-1-VERIFICATION-LOG.md` + `docs/SESSION-LOGS/VENUE-ADMIN-UI-A1-STAGE-2-VERIFICATION-LOG.md` + `docs/SESSION-LOGS/VENUE-ADMIN-UI-A3-VERIFICATION-LOG.md` (the per-stage verification record).
